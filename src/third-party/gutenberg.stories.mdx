...
The quote block adds class of `wp-block-quote` to a `blockquote` element. It has
a control to add an optional `is-style-large` class.
...
(@TODO - Move anything like this to a top-level content section.)
Controls enable setting of the wrapper element's `background-color` and `color`.

The base class for a group with a background-color is `has-background` and the
selection may add a modified class like `has-subtle-background-background-color`.
Setting a text `color` will add the class `has-text-color` and may add a modified class like
`has-secondary-color`.

If user-defined colors are chosen, no modifier classes will
be added. Instead, inline styles will set `color` and/or `background-color`.
...
<Preview>
  <Story name="core/group" height="500px">
...
an `alignwide` (horizontal padding) or `alignfull` (no horizontal padding) class.
Another control toggles the class `is-stacked-on-mobile`.

<Preview>
  <Story name="core/media-text" height="500px">
    {`<div class="wp-block-media-text alignwide is-stacked-on-mobile has-media-on-the-right is-vertically-aligned-center has-background has-subtle-background-background-color">
        <figure class="wp-block-media-text__media">
          <img src="https://s.w.org/images/core/5.3/Biologia_Centrali-Americana_-_Cantorchilus_semibadius_1902.jpg" alt="" />
        </figure>
...
<Preview>
  <Story name="core/preformatted" height="350px">
...
Color controls allow the setting of pre-defined or custom `background-color` and
`color`. A styles control adds one of two wrapper classes, `is-style-default` or
...
<Preview>
  <Story name="core/pullquote" height="200px">
...
and adds a `has-text-align-` class to each affected cell.

Style controls add one of 2 classes, `is-style-default` or `is-style-stripes` to
the parent wrapper, for controlling zebra-striping on alternate rows.

Another control optionally adds the `has-fixed-layout` class to the `table` element.

<Preview>
  <Story name="core/table" height="250px">
...
Style controls add `is-style-default`, `is-style-wide` or `is-style-dots` classes.

The color control, if used, adds both `has-text-color` and `has-background` classes,
as well as modifier classes for predefined color choice or inline styles for user-defined
color selection.

<Preview>
  <Story name="core/separator" height="100px">
...
<Preview>
  <Story name="core/verse" height="300px">
...
The audio block wraps an `audio` element and an optional `figcaption` element in
a `figure` with class `wp-block-audio`

<Preview>
  <Story name="core/audio" height="200px">{`
...
content and adding `is-type-` and `is-provider-` classes to the parent wrapper
element.
...
<Preview>
  <Story name="core/embed" height="300px">
    {`<figure
...
<Preview>
  <Story name="core/spacer" height="200px">
