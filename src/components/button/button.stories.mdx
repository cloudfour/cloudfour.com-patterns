...
{% embed '@cloudfour/components/button/button.twig' with { content_end_icon: 'search' } %}
...
`content_start` or `content_end` Button component block.
...
- `aria-expanded` (boolean): Sets the button's `aria-expanded` attribute.
