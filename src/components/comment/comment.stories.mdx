...
import { initCommentWithRepliesEnabled } from './comment.ts';
...
You can run `initCommentWithRepliesEnabled` to initialize the ability to reply to comments.

(This is not necessary for pages with comments disabled, like older posts.)

```js
import { initCommentWithRepliesEnabled } from 'path/to/comment.js';
initCommentWithRepliesEnabled();
```
