...
// eslint-disable-next-line @cloudfour/import/no-webpack-loader-syntax
import promoDemoSource from '!!raw-loader!./demo/promo.twig';
import promoDemo from './demo/promo.twig';
...
In this example, we're using it for a Promo.

You may want to add some of our [spacing utility classes](/docs/utilities-spacing--margin) to the media object to compensate for the rotation.
In this example, we've used `u-space-inline-end-1` and `u-space-inline-start-n4` to add a bit more space between the image and the text, and keep the image from hitting the left side of the viewport on small screens.

<Canvas>
  <Story
    name="Jaunty"
    parameters={{ docs: { source: { code: promoDemoSource } } }}
  >
    {promoDemo}
  </Story>
</Canvas>
