...
By default, content will appear middle-aligned with the object. If the content exceeds the object's height, they will appear top-aligned. You can change the alignment so the content and object remain top aligned using the modifier `o-media--alignStart`.

<Canvas>
  <Story
    name="Alignment"
    args={{ class: 'o-media--alignStart' }}
    parameters={{
      docs: {
        transformSource: imageDemoTransformSource,
      },
