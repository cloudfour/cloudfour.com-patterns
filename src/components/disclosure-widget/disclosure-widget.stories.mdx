...
import disclosureWidgetDemo from './demo/disclosure-widget-demo.twig';
...
<Canvas>
  <Story name="Basic usage">{(args) => templateStory(args)}</Story>
  <Story name="Another">{(args) => disclosureWidgetDemo(args)}</Story>
</Canvas>
