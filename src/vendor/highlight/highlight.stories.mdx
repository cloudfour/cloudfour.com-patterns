...
const sampleStory = (args) =>
  sampleDemo({ ...args, ...samples[args.language || 'html'] });
const sampleTransformSource = (src) => src;

<Meta
  title="Vendor/Highlight"
  parameters={{
    docs: {
      transformSource: sampleTransformSource,
    },
    layout: 'fullscreen',
  }}
  argTypes={{
...
For performance, it's recommended to apply syntax highlighting on the server:
...
<Canvas>
  <Story name="Example" args={{ language: 'html' }}>
    {sampleStory.bind()}
  </Story>
</Canvas>
