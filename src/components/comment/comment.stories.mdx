...
You can run `initCommentReplyForm` to initialize a comment's reply form. Comments with reply forms will have the
...
const comments = document.querySelectorAll('.js-comment-with-reply-form');
for (const comment of comments) {
  initCommentReplyForm(comment);
}
```
