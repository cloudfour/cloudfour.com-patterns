...
- `comment`: A comment object containing the following properties:
  ...
  - `demo_cloud_four_member`: A boolean property that indicates if the commentor is a Cloud Four member. (Name will be changed in the future.)
  - `demo_post_author`: A boolean property that indicates if the commentor is the post author. (Name will be changed in the future.)
...
```js
import { initComments } from 'path/to/comment.js';
initComments();
```
