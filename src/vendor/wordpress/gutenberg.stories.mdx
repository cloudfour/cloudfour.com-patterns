...
Additionally, the Advanced → Additional CSS Classes input can be used to generated ruled and numeric tables by adding `c-table--ruled` and/or `c-table--numeric`. This will add the additional classes to the `<figure>` element, which differs from how our `c-table` pattern is set up, but the styled outcome will be the same.
