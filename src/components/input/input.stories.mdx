  ...
  <Story name="Elastic Textarea" height="200px">
    {() => {
      ...
      useEffect(() => {
        ...
        // Run the create function on each element, save as array
        const elasticTextareaArray = [...elasticTextareaNodeList].map(
          (textarea) => {
            return createElasticTextarea(textarea);
          }
        );
        // Destroy the elastic text areas
        // TODO: remove this once validated this works
        elasticTextareaArray.forEach((textarea) => {
          textarea.destroy();
        });
      }, []);
      return textDemo({
