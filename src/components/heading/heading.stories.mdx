...
The default font weight for headings is normally defined by heading level. This modifier will change the heading to `font-weight.$light`. We do this so we can do enforce a visual hierarchy without sacrificing the importance of headings and not drowning out other content.
