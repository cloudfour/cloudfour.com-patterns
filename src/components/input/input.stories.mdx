...
If you pass the `label` property, a label will automatically be created and associated with the input element.

<Canvas>
  <Story
    name="Text Elements With Label"
    height="200px"
    argTypes={{
      type: {
        type: { name: 'enum' },
      ...
      value: { type: { name: 'string' } },
      label: { type: { name: 'string' }, defaultValue: 'Label' },
      placeholder: {
        type: { name: 'string' },
        defaultValue: 'Placeholder text…',
      },
