...
import demoDestroyReinitTemplate from './demo/destroy-reinit.twig';
...
The Subscribe component allows you to programmatically disable/enable the JavaScript functionality via its `destroy()` and `reinit()` methods. When in its "destroyed" UI state, the following will occur:
