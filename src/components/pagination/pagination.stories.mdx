...
import mockPaginationData from './demo/mock-pagination-data.ts';
...
- Said element includes an `aria-labelledby` attribute, which maps to the ID of a [visually hidden](/iframe.html?id=utilities-display--hidden-visually&viewMode=docs#hidden-visually) heading element within. We use `aria-labelledby` instead of `aria-label` to improve [compatibility with translation tools](https://heydonworks.com/article/aria-label-is-a-xenophobe/).
