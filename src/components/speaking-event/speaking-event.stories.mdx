...
const speakingEventStory = (args) => {
  if (!args.datetime) {
    args.datetime = '2021-10-03';
  }
  if (!args.speakers) {
    args.speakers = ['Arianna'];
  }
  if (!args.event) {
    args.event = 'Smashing Conference';
  }
  if (!args.location) {
    args.location = 'Freiburg, Germany';
  }
