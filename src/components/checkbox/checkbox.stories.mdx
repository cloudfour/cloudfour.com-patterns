import { Story, Canvas, Meta } from '@storybook/addon-docs/blocks';
import withLabelDemo from './demo/with-label.twig';
import './demo/styles.scss';

<Meta
  title="Components/Checkbox"
  argTypes={{
    disabled: { type: { name: 'boolean' } },
  }}
/>

# Checkbox

`c-checkbox` may be applied to any `<input type="checkbox">` element. They are larger than native checkboxes and more visually consistent with our [input](/docs/components-input--text-elements) and [button](/docs/components-button--elements) components.

<Canvas>
  <Story
    name="Enabled"
    height="120px"
    args={{ disabled: false }}
    parameters={{
      docs: {
        source: {
          code: `<label>
  {% include '@cloudfour/components/checkbox/checkbox.twig' %}
  Unchecked
</label>
<label>
  {% include '@cloudfour/components/checkbox/checkbox.twig' with { checked: true } %}
  Checked
</label>`,
        },
      },
    }}
  >
    {(args) => withLabelDemo(args)}
  </Story>
</Canvas>

Checkboxes appear more subtle and less actionable when the `disabled` attribute is set.

<Canvas>
  <Story
    name="Disabled"
    height="120px"
    args={{ disabled: true }}
    parameters={{
      docs: {
        source: {
          code: `<label>
  {% include '@cloudfour/components/checkbox/checkbox.twig' with { disabled: true } %}
  Unchecked
</label>
<label>
  {% include '@cloudfour/components/checkbox/checkbox.twig' with { disabled: true, checked: true } %}
  Checked
</label>`,
        },
      },
    }}
  >
    {(args) => withLabelDemo(args)}
  </Story>
</Canvas>
